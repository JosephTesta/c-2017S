//what can go wrong that we need exception for
//in copy contructor you might be out of memory. you would get badalloc. don't need to do anything with it
//in add, 
...
public:
	...
	void add(const T& v) throw (bad_alloc) {
		if (used >= capacity)
			grow();
		data[used++] = v;
	}
	...
	//x[i] not x [] i. not a friend
	T operator [](int i) const { //they can't change it this way .read only version. compiler knows difference
		return data[i];
	}

	T& operator [](int i) { // this is the read/write version. will write to the array
		return data[i]l 
	}
};
...
int main() {
	...
	a.add("abc");
	a.add("def");
	//cout << a[10000] << '\n'; //we can either let this break, or provide a check and do something
